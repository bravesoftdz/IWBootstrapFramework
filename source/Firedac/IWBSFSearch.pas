...
type
 ...
 //   FDsQryBusca: TDataSource; // dataset que mostrara os resultados no dropdown
    ...
    FBtnAdd: TIWBSButton; // Botão personalizavel lateral direita
    FKey: Word;           // ----------v
    FShift: TShiftState;
  ...
  //  procedure SetTopLabelCss(const Value: string);
    ...
    function FindParentForm: TIWForm;
    procedure PosicionaLista;
    procedure AjustaLarguraColunasDbGrid(Grid: TIWBSTable);
 ...
 //   procedure SetTopLabelCss(const Value: string);
    ...
    procedure DoDropDownData(aApplication: TIWApplication;
...
function TIWBSFSearch.FindParentForm: TIWForm;
var
  CompTest: TControl;
begin
  Result := nil;
  try
    CompTest := self;
    while Assigned(CompTest) and (not(CompTest is TIWForm)) do
      ...
      begin
        if TComponent(self).Owner is TFrame then
          begin
            ...
            while (not(CompTest is TIWForm)) and (CompTest <> nil) do
              CompTest := TControl(TComponent(CompTest).Owner);
          end;
      end;
    if CompTest <> nil then
      if CompTest is TIWForm then
        Result := CompTest as TIWForm;
  except

  end;
end;

procedure TIWBSFSearch.AguardaTerminarDigitacao(Sender: TObject);
begin
  BuscaLista(self, FKey, FShift); // executa busca lista antes do evento da classe pai]
end;

procedure TIWBSFSearch.BtnAddOnClick(Sender: TObject; aParams: TStringList);
begin
  if Assigned(FOnBtnClick) then
    FOnBtnClick(self);
end;

procedure TIWBSFSearch.BuscaLista(Sender: TObject; var Key: Word; Shift: TShiftState);
var
...
//  FDsQryBusca.DataSet         := FQryBusca;
  ...
  if FQryBusca.RecordCount > 0 then
    begin
      ...
      IWBSExecuteAsyncJScript(gGetWebApplicationThreadVar,'$("#'+HTMLName+'DROP").bootstrapTable(''refresh'');', False, True);
...
procedure TIWBSFSearch.PosicionaLista;
var
  Posicao: TWinControl;
  i: Integer;
  GridWidth: Integer;
...
procedure TIWBSFSearch.AjustaLarguraColunasDbGrid(Grid: TIWBSTable);
const
  DEFBORDER = 10;
var
  temp, n: Integer;
...
constructor TIWBSFSearch.Create(AOwner: TComponent);
var
  i: Integer;
begin
  ...
  // cria o botão
  FBtnAdd := TIWBSButton.Create(self);
...
procedure TIWBSFSearch.DoOnAsyncKeyUp(aParams: TStringList);
var
  ParentFormRec: TIWForm;
  KeyReceived: Word;
...
procedure TIWBSFSearch.DoOnAsyncSelect(aParams: TStringList);
begin
   DropDownVisible:=False;
...
destructor TIWBSFSearch.Destroy;
begin
  // FQryBusca.Free;
  // Fedit.Free;
  // FQryBusca.Free;
...
procedure TIWBSFSearch.OnClickLista(Sender: TObject; aParams: TStringList);
var
  Enter: Word;
begin
...
procedure TIWBSFSearch.VoltaParaFedittxt(Sender: TObject; aParams: TStringList);
var
  ParentForm: TIWForm;
begin
  ParentForm := FindParentForm;
  if ParentForm <> nil then
    ParentForm.WebApplication.ShowMessage('VoltaParaFedittxt Params:' + aParams.Text);
  {
...
function TIWBSFSearch.GetResultBusca: TFDQuery;
begin
  Result := FQryBusca;
end;

procedure TIWBSFSearch.InternalRenderHTML(const AHTMLName: string;
...
begin
  inherited;
  // Render Dropdown
   DropTag := TIWHTMLTAG.CreateTag('table');
...
begin
  ...
  AContext.WebApplication.RegisterCallBack(AHTMLName+'.DoOnAsyncSelect', DoOnAsyncSelect);

end;

function TIWBSFSearch.RenderHTML(Acontext: TIWCompContext): TIWHTMLTAG;
var
  DropTag: TIWHTMLTAG;
  //LabelStyle: string;
  ajaxcall: string;
...
procedure TIWBSFSearch.SetAlturaLinhaLista(const Value: Integer);
begin
...
procedure TIWBSFSearch.SetAlturaLista(const Value: Integer);
begin
  if FAlturaLista <> Value then
    FAlturaLista := Value;
end;

procedure TIWBSFSearch.SetCamposBusca(const Value: TCamposBusca);
begin
  FCamposBusca.Assign(Value);
end;

procedure TIWBSFSearch.SetColunas(const Value: TColunas);
begin
  FColunas.Assign(Value);
end;

procedure TIWBSFSearch.SetDropDownVisible(const Value: Boolean);
var
  DropCss:string;
begin
  if Value <> FDropDownVisible then
    begin
      SetAsyncVisible(gGetWebApplicationThreadVar, HTMLName + 'DROP', Value, FDropDownVisible);
      ...
      IWBSExecuteAsyncJScript(gGetWebApplicationThreadVar,'var styles =' + DropCss +';  $(".bootstrap-table:has(#' + HTMLName +'DROP)").css(styles);', False, True);
    end;
end;

procedure TIWBSFSearch.SetSelecionaAoClicar(const Value: Boolean);
begin
...
begin
  ...
  try
    ...
    OptTxt.Values['url']              := '"'+ IWBSRegisterRestCallBack(gGetWebApplicationThreadVar, HTMLName +'.dataurl', DoDropDownData) + '"';
...
procedure TIWBSFSearch.DoDropDownData(aApplication: TIWApplication;
  aRequest: THttpRequest; aReply: THttpReply; aParams: TStrings);
...
begin
  ...
  try
    ...
    aReply.WriteString('{"total": ' + IntToStr(QryResultBusca.RecordCount) + ', "rows": [' +
      data + ']}');
  finally
...
function TsMyListGrid.RenderHTML(Acontext: TIWCompContext): TIWHTMLTAG;
begin
  if Visible then
    Result := inherited RenderHTML(Acontext)
  else
